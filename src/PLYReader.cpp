...
bool PLYReader::loadHeader(ifstream& fin, int& nVertices, int& nFaces)
{
    ...
    {
        if (strncmp(line, "element vertex", 14) == 0) nVertices = atoi(&line[15]);
        fin.getline(line, 100);
        if (strncmp(line, "element face", 12) == 0) nFaces = atoi(&line[13]);
    }
...
void PLYReader::addModelToMesh(const vector<float>& plyVertices, const vector<int>& plyTriangles, TriangleMesh& mesh)
{
    ...
    for (i = 0; i < plyVertices.size(); i += 3) mesh.addVertex(glm::vec3(plyVertices[i], plyVertices[i + 1], plyVertices[i + 2]));
    for (i = 0; i < plyTriangles.size(); i += 3) mesh.addTriangle(plyTriangles[i], plyTriangles[i + 1], plyTriangles[i + 2]);
}
