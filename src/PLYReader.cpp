...
void PLYReader::loadVertices(ifstream &fin, int nVertices,
                             vector<float> &plyVertices) {
  int i;
  float value;
...
void PLYReader::loadFaces(ifstream &fin, int nFaces,
                          vector<int> &plyTriangles) {
  int i, tri[3];
...
void PLYReader::rescaleModel(vector<float> &plyVertices) {
  ...
  float largestSize =
      max(size[1][0] - size[0][0],
          max(size[1][1] - size[0][1], size[1][2] - size[0][2]));
...
void PLYReader::addModelToMesh(const vector<float> &plyVertices,
  ...
  for (i = 0; i < plyVertices.size(); i += 3)
    mesh.addVertex(
        glm::vec3(plyVertices[i], plyVertices[i + 1], plyVertices[i + 2]));
